    ...
    public static void main(String[] args) {
        try {
            final CanalRocketMQClientExample rocketMQClientExample = new CanalRocketMQClientExample(nameServers,
                    topic,
                    groupId);
            logger.info("## Start the rocketmq consumer: {}-{}", AbstractRocketMQTest.topic, AbstractRocketMQTest.groupId);
    ...
    private void process() {
        ...
        while (running) {
            try {
                ...
                connector.subscribe();
                while (running) {
                    List<Message> messages = connector.getListWithoutAck(5000L, TimeUnit.MILLISECONDS); // 获取message
